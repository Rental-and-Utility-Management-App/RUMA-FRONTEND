...
@Component({
  selector: 'app-manager-invoice-list',
  standalone: true,
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              @for (inv of filteredInvoices(); track inv.id) {
                ...
                >
                  ...
                  <!-- Chỉ manager mới thấy nút này (đã ở trang manager), và chỉ khi
                       hóa đơn CHƯA có bất kỳ khoản thanh toán nào (paid_amount == 0)
                       và chưa bị hủy — khớp với điều kiện backend enforce. -->
                  @if (canCancel(inv)) {
                    <button
                      type="button"
                      (click)="$event.preventDefault(); $event.stopPropagation(); submitCancel(inv)"
                      [disabled]="cancellingId() === inv.id"
                      class="relative mt-2 w-full rounded-full border border-[#9A3412] px-4 py-2 text-xs font-semibold text-[#9A3412] transition hover:bg-[#F4D9D2] disabled:opacity-60"
                    >
                      {{ cancellingId() === inv.id ? 'Đang hủy...' : 'Hủy hóa đơn' }}
                    </button>
                  }
                </a>
              } @empty {
...
export class ManagerInvoiceListPage {
  ...
  // ---- Hủy hóa đơn ----
  // Lưu id đang hủy (thay vì 1 boolean chung) để disable đúng nút của đúng
  // card đang xử lý, không disable nhầm toàn bộ danh sách khi có nhiều thẻ.
  ...
  // Điều kiện hiện nút "Hủy hóa đơn": khớp với luật backend — chỉ cho hủy
  // khi CHƯA ghi nhận thanh toán nào (paid_amount == 0) và hóa đơn chưa hủy.
  canCancel(inv: any): boolean {
    return inv.status !== 'cancelled' && (inv.paid_amount ?? 0) === 0;
  }
  ...
  // ---- Hủy hóa đơn: POST /invoices/{id}/cancel ----
  // Manager hủy 1 hóa đơn tạo sai. CHỈ cho hủy khi chưa ghi nhận thanh toán
  // nào (paid_amount == 0). Hóa đơn hủy vẫn được giữ lại (soft-cancel).
  async submitCancel(inv: any) {
    const ok = await this.confirm.ask({
      title: 'Xác nhận hủy hóa đơn',
      message: `Hủy hóa đơn kỳ ${inv.month}/${inv.year} của phòng ${this.roomLabel(inv)}? Hành động này không thể hoàn tác.`,
    ...
    try {
      await this.invoicesService.cancel(inv.id);
      this.invoices.reload();
      this.toast.success('Hủy hóa đơn thành công.');
    } catch (err: any) {
      this.toast.error(err?.error?.message ?? err?.message ?? 'Hủy hóa đơn thất bại.');
    } finally {
